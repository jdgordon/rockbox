...
static bool skin_initialising = true;
...
static struct gui_skin {
    ...
    unsigned char buffer[SKIN_BUFFER_SIZE];
...
struct gui_wps *skin_get_gwps(enum skinnable_screens skin, enum screen_type screen)
{
    if (!loading_a_sbs && skins[skin][screen].data.wps_loaded == false)
    {
        ...
        cpu_boost(true);
        if (strcmp(buf, skins[skin][screen].filename))
        {
            skin_data_free_buflib_allocs(&skins[skin][screen].data);
            skin_buffer_init(skins[skin][screen].buffer, SKIN_BUFFER_SIZE);
